...
export const mailRouter = createTRPCRouter({
    ...
    getThreads: protectedProcedure.input(z.object({
        ...
        // const messages = await nylas.messages.list({ identifier: account.id, queryParams: {
        //    select: ' from,body,bodySnippet,emailLabel,subject,sysLabels,id,sentAt',
        //    limit: 15
        // } });
    ...
    sendEmail: protectedProcedure.input(z.object({
        ...
        threadId: z.string().optional(),
            ...
            if (!acc || !acc.token) {
                throw new Error('Failed to authorize account access or invalid token');
            }
            const account = new Account(acc.token);
            console.log('sendmail', input)
            console.log('Account initialized with token:', acc.token);
